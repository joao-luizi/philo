...
static bool handle_thread_error (int status, t_opcode opcode)
{
    if (status == 0)
        ...
        return (printf(R "The caller does not have appropriate permissions" RST), false);
    else if (status == EINVAL && opcode == CREATE)
        return (printf(R "The value specified by attr is invalid." RST), false);
    else if (status == EINVAL && (opcode == JOIN || opcode == DETACH))
        return (printf(R "The value specified by thread is not joinable" RST), false);
    else if (status == ESRCH)
        return (printf(R "No thread could be found corresponding to that specified by the given thread ID, thread" RST), false);
    else if (status == EDEADLK)
        return (printf(R "A deadlock was detected or the value of thread specifies the calling thread" RST), false);
    else
