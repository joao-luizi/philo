...
/*   Updated: 2025/03/27 11:09:43 by joaomigu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
* @brief Writes the philosopher's current state to the console in a 
	thread-safe manner.
*
* This function locks the shared write mutex before printing the 
	philosopher's state.
* It first checks if the simulation has ended and returns early if so. If the
	 simulation
 ...
 *
	...
	* @return (true if the message was written successfully or if simulation
	 ended);
...
/**
	...
	* @brief Desynchronizes philosophers to prevent simultaneous resource access
	 at simulation start.
 *
 ...
 *
	...
	* This helps reduce contention for forks in the initial phase of the 
	simulation.
 *
...
/**
	...
	* @brief Entry point for a single philosopher thread when only one 
	philosopher exists.
 *

	* Handles the special case of a simulation with just one philosopher. It 
	initializes
 * `last_meal_time`, attempts to take a fork,
	and waits for the simulation to end.
 *
 ...
 * @return (NULL Always returns NULL); designed for pthread usage.
	...
	* @note The philosopher takes the first fork and waits for `end_simulation`
	 to become true.
 ...
 *
	...
	* Retrieves the current simulation time and stores it as the philosopher's
	 last meal time.
 * Also checks if the simulation has already ended.
 *
 * @param philo Pointer to the philosopher to initialize.

	* @param local_end_simulation Pointer to a boolean that will store the 
...
/**
	...
	* @brief Entry point for each philosopher thread in the multi-philosopher
	 simulation.
 *
 * Sets up initial state, desynchronizes execution to reduce contention,
	and enters
 * the main philosopher routine of eating, sleeping, and thinking.
 *

	* The loop continues until the shared `end_simulation` flag is set or the
	 philosopher
 * becomes full (if a meal count limit is used).
 *
 * @param args Pointer to a `t_philo` struct representing the philosopher.
 * @return (NULL Always returns NULL); designed for pthread usage.
 * @see philo_setup, de_sync_philos, safe_get, philo_eat, write_states,
	custom_sleep, philo_think
 */
