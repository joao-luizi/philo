...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/03/23 19:01:22 by joaomigu          #+#    #+#             */
/*   Updated: 2025/03/23 19:06:06 by joaomigu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (status == EACCES)
		error_exit("The caller does not have permission to \
            unlink this semaphore.", table);
...
static void	handle_sem_error_create(int status, t_table *table)
{
	if (status == 0)
		return ;
	if (status == EACCES)
		error_exit("The semaphore exists,\
			but the caller does not have permission to open it.", table);
	else if (status == EEXIST)
		error_exit("Both O_CREAT and O_EXCL were specified,\
			but a semaphore with this name already exists.", table);
	else if (status == EINVAL)
		error_exit("Invalid semaphore name format or value is \
            greater than SEM_VALUE_MAX.", table);
	else if (status == EMFILE)
		error_exit("The per-process limit on open file descriptors\
             has been reached.", table);
	else if (status == ENAMETOOLONG)
		error_exit("The semaphore name is too long.", table);
	else if (status == ENFILE)
		error_exit("The system-wide limit on total open files has \
            been reached.", table);
	else if (status == ENOENT)
		error_exit("The semaphore does not exist, and O_CREAT was \
            not specified, or the name wasn't well-formed.", table);
	else if (status == ENOMEM)
		error_exit("Insufficient memory to create the semaphore.", table);
}

static void	handle_sem_error_lock(int status, t_table *table)
{
	if (status == EINVAL)
		error_exit("Specified semaphore is not valid.", table);
	else if (status == EOVERFLOW)
		error_exit("The maximum allowable value for a semaphore \
            would be exceeded.", table);
	else if (status == EINTR)
		error_exit("The semaphore wait operation was interrupted \
            by a signal.", table);
}
 ...
 * @param opcode The semaphore operation being performed (e.g., LOCK, UNLOCK,
	CREATE).
 * @param table The table structure used for resource cleanup.
 */
static void	handle_sem_error(int status, t_sem_action opcode, t_table *table)
{
	...
	if (opcode == SEM_LOCK || opcode == SEM_UNLOCK)
		handle_sem_error_lock(status, table);
	...
	else if (opcode == SEM_INIT)
	{
		...
		if (status == EINVAL)
			...
			error_exit("The system does not support unnamed semaphores.",
				table);
	}
	else
 ...
 *
 * This function performs various semaphore operations (e.g., LOCK,
 * UNLOCK,
	CREATE,
 * UNLINK, CLOSE) and handles errors appropriately. For CREATE,
	it initializes the
 * semaphore and updates the pointer.
 *
 * @param sem A double pointer to the semaphore being operated on.

	* @param name The name of the semaphore (used for CREATE and
		UNLINK operations).
 * @param opcode The semaphore operation to perform (e.g., LOCK,
 UNLOCK,
	CREATE).
 * @param table The table structure used for resource cleanup.
 */
