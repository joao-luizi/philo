...
{
    safe_sem_handle(table->console_sem, "/console_sem", UNLINK, table);
    safe_sem_handle(table->forks_sem, "/forks_sem", UNLINK, table);
    safe_sem_handle(table->end_simulation, "/end_simulation", UNLINK, table);
    if (table->pids)
        free(table->pids);
}
...
static void table_init(t_table *table)
{
    ...
    if (table->console_sem == SEM_FAILED)
        handle_sem_error(errno, CREATE, table);
    safe_sem_handle(table->forks_sem, "/forks_sem", UNLINK, table);
